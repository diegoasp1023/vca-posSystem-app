...
async def test_list_menu_categories_requires_auth(client):
    response = await client.get("/api/menu-categories")

    assert response.status_code == 401


async def test_list_menu_categories_ordered_by_sort_order(db_session, admin_client):
    ...
    response = (await admin_client.get("/api/menu-categories")).json()
