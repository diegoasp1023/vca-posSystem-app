...
async def test_list_tabs_as_gerente(gerente_client):
    response = await gerente_client.get("/api/tabs")
...
async def test_list_tabs_excludes_paid_tabs_from_previous_days(db_session, gerente_client):
...
async def test_create_tab(gerente_client):
...
async def test_update_open_tab(db_session, gerente_client):
    tab = await make_tab(db_session)

    response = await gerente_client.patch(
...
async def test_delete_open_tab(db_session, gerente_client):
    tab = await make_tab(db_session)

    response = await gerente_client.delete(f"/api/tabs/{tab.id}")

    assert response.status_code == 204


async def test_delete_paid_tab_is_rejected(db_session, gerente_client):
...
async def test_add_menu_item_to_tab_snapshots_name_and_price(db_session, gerente_client):
    tab = await make_tab(db_session)
...
async def test_adding_same_item_twice_increments_quantity(db_session, gerente_client):
    tab = await make_tab(db_session)
    menu_item = await make_menu_item(db_session)

    await gerente_client.post(
        f"/api/tabs/{tab.id}/items",
...
async def test_add_item_rejects_inactive_menu_item(db_session, gerente_client):
    tab = await make_tab(db_session)
...
async def test_add_item_rejects_menu_item_without_price(db_session, gerente_client):
    tab = await make_tab(db_session)
...
async def test_update_tab_item_quantity(db_session, gerente_client):
    tab = await make_tab(db_session)
...
async def test_delete_tab_item(db_session, gerente_client):
    tab = await make_tab(db_session)
    item = await make_tab_item(db_session, tab)

    response = await gerente_client.delete(f"/api/tabs/{tab.id}/items/{item.id}")

    assert response.status_code == 200
    assert response.json()["items"] == []


async def test_cannot_edit_items_on_paid_tab(db_session, gerente_client):
    tab = await make_tab(db_session, status="paid")
    item = await make_tab_item(db_session, tab)
...
async def test_pay_tab(db_session, gerente_client):
    tab = await make_tab(db_session)
...
async def test_pay_tab_with_no_items_is_rejected(db_session, gerente_client):
    tab = await make_tab(db_session)
    method = await make_tab_payment_method(db_session)
...
async def test_pay_already_paid_tab_is_rejected(db_session, gerente_client):
    tab = await make_tab(db_session, status="paid")
    method = await make_tab_payment_method(db_session)
...
async def test_reopen_paid_tab(db_session, gerente_client):
    method = await make_tab_payment_method(db_session)
    tab = await make_tab(db_session, status="paid", payment_method_id=method.id)

    response = await gerente_client.post(f"/api/tabs/{tab.id}/reopen")
...
async def test_reopen_open_tab_is_rejected(db_session, gerente_client):
    tab = await make_tab(db_session)
